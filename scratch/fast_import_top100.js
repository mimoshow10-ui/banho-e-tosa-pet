...
async function importarProdutoCompleto(token, itemBling) {
  try {
    ...
    if (!prodCompleto && itemBling.sku) {
      try {
        const res = await fetchWithTimeout(`https://api.bling.com.br/Api/v3/produtos?codigo=${encodeURIComponent(itemBling.sku)}`, {
          headers: { 'Authorization': 'Bearer ' + token }
        });
        if (res.ok) {
          const json = await res.json();
    ...
    if (!prodCompleto) {
      console.log(`⚠️ Detalhes indisponíveis para produto: ${itemBling.sku || itemBling.nome}`);
      return false;
    }
    ...
    const skuCod = prodCompleto.codigo || prodCompleto.gtin || itemBling.sku || null;
    ...
    if (prodExistente?.id) {
      ...
      else console.log(`✓ [ATUALIZADO] ${prodCompleto.nome} (SKU: ${skuCod || 'N/A'})`);
    } else {
      ...
      else console.log(`✨ [SINCRONIZADO] ${prodCompleto.nome} (SKU: ${skuCod || 'N/A'})`);
    }
...
async function runMasterImport() {
  console.log('🚀 Iniciando identificação e importação dos 100 produtos PET mais vendidos no Bling...');
  ...
  console.log('🔎 Buscando produtos PET ativos no Bling...');
  let pag = 1;
  let todosProds = [];
  ...
  console.log(`🐾 Encontrados ${petProds.length} produtos PET ativos no Bling.`);

  const selecionados = petProds.slice(0, 100);

  console.log(`\n📦 INICIANDO IMPORTAÇÃO DOS ${selecionados.length} PRODUTOS PET SELECIONADOS PARA O SITE:\n`);

  let concluidos = 0;
  for (let i = 0; i < selecionados.length; i++) {
    const item = selecionados[i];
    console.log(`[${i + 1}/${selecionados.length}] Processando: ${item.nome} (SKU: ${item.codigo || 'N/A'})`);
  ...
  console.log(`\n==================================================`);
  console.log(`🎉 IMPORTAÇÃO DOS 100 PRODUTOS PET FINALIZADA!`);
  console.log(`Total de produtos sincronizados no site: ${concluidos}/${selecionados.length}`);
}
