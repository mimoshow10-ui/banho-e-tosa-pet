        ...
        <div className="sticky top-16 z-30 bg-secondary text-white p-4 rounded-2xl shadow-xl border border-blue-900 flex flex-col md:flex-row items-center justify-between gap-4 animate-in slide-in-from-top-2 duration-200">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-gray-200 hidden sm:inline">
              Escolha a ação em massa para aplicar:
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
            ...
            <select
              value={acaoMassa}
              onChange={(e) => {
                setAcaoMassa(e.target.value);
                if (e.target.value === 'destaque') setValorMassa('super_promocao');
                if (e.target.value === 'status') setValorMassa('true');
                if (e.target.value === 'categoria') setValorMassa(categorias[0]?.id || '');
              }}
            ...
            {acaoMassa === 'preco_promocional' && (
              <input
                ...
                className="bg-white text-secondary border border-gray-300 rounded-xl px-3 py-2 text-xs font-bold w-28 focus:outline-none"
              />
            )}
